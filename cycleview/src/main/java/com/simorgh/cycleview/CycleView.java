...
//            btnChangeTextPaint.setShadowLayer(dp2px(20), dp2px(10), dp2px(10), btnChangeBkgShadowColor);
    ...
    public CycleView(Builder builder, CycleData cycleData, OnDayChangedListener onDayChangedListener, OnButtonClickListener onButtonClickListener) {
        super(builder.context);
        setClueDate(cycleData, onDayChangedListener, onButtonClickListener);
    }

    public CycleView(Builder builder, CycleData cycleData, ViewData viewData, OnDayChangedListener onDayChangedListener, OnButtonClickListener onButtonClickListener) {
        super(builder.context);
        setClueAndViewData(cycleData, viewData, onDayChangedListener, onButtonClickListener);
    }
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.CycleView);
        Resources resources = getResources();

        //circle colors
        mainCircleColor = typedArray.getColor(R.styleable.CycleView_main_circle_color, resources.getColor(R.color.type_red));
        mediumCircleColor = typedArray.getColor(R.styleable.CycleView_medium_circle_color, resources.getColor(R.color.type_red));
        smallCircleTypeRedColor = typedArray.getColor(R.styleable.CycleView_small_circle_color_type1, resources.getColor(R.color.type_red));
        smallCircleTypeGreenColor = typedArray.getColor(R.styleable.CycleView_small_circle_color_type2, resources.getColor(R.color.type_green));
        smallCircleTypeGreen2Color = typedArray.getColor(R.styleable.CycleView_small_circle_color_type2, resources.getColor(R.color.type_green));
        smallCircleTypeGrayColor = typedArray.getColor(R.styleable.CycleView_small_circle_color_type3, resources.getColor(R.color.type_gray));
        smallCircleTypeYellowColor = typedArray.getColor(R.styleable.CycleView_small_circle_color_type4, resources.getColor(R.color.type_yellow));


        //btn change
        btnChangeBkgColor = typedArray.getColor(R.styleable.CycleView_main_circle_color, resources.getColor(R.color.white));
    ...
    public void showToday(Calendar today) {
        ...
        int diffDays = (int) CalendarTool.getDaysFromDiff(temp, cycleData.getStartDate());
        if (diffDays >= 0) {
...
//            btnChangeBkgPaint.setShadowLayer(dp2px(BTN_CHANGE_SHADOW_RADIUS), dp2px(BTN_CHANGE_SHADOW_X_OFFSET), dp2px(BTN_CHANGE_SHADOW_Y_OFFSET), btnChangeBkgShadowColor);
    ...
    private Paint getCurrentDayPaint() {
        ...
        if (getCurrentDay() <= cycleData.getRedCount()) {
            return smallCircleTypeRedPaint;
        } else if (getCurrentDay() <= cycleData.getTotalDays()) {
            if (getCurrentDay() >= cycleData.getGreenStartIndex() && getCurrentDay() <= cycleData.getGreenEndIndex()) {
                if (getCurrentDay() == cycleData.getGreen2Index()) {
                    return smallCircleTypeGreenPaint;
                }
                return smallCircleTypeGreenPaint;
            } else if (getCurrentDay() >= cycleData.getYellowStartIndex() && getCurrentDay() <= cycleData.getYellowEndIndex()) {
    ...
    private void drawSmallCircles(Canvas canvas) {
        float dayAng;
        for (int i = 1; i <= cycleData.getTotalDays(); i++) {
...
//                    Log.d("debug13", "bad angle");
    ...
    private void setClueAndViewData(CycleData cycleData, ViewData viewData, OnDayChangedListener onDayChangedListener, OnButtonClickListener onButtonClickListener) {
    ...
    public void setClueDate(CycleData clueDate, OnDayChangedListener onDayChangedListener, OnButtonClickListener onButtonClickListener) {
        this.cycleData = clueDate;
        this.onDayChangedListener = onDayChangedListener;
        invalidate();
    }
    ...
    private float calculateAngleForDay(int dayNumber) {
        ...
        float angleUnit = (360 - 2 * offsetAngle) / (float) cycleData.getTotalDays();
    ...
    public void onViewDataChanged(String weekDayName, String optionalText, String mainDayNumber, String monthDayNumber, String monthName, boolean isOptionalVisible, int today) {
        if (cycleData != null) {
            if (today > cycleData.getTotalDays()) {
                today = 1;
            }
        }
        selectedDay = today;
